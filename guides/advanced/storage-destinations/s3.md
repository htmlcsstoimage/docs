...
You can also automate configuration through the [Storage destinations API](/management-api/storage-destinations/) or [MCP tools](/integrations/mcp/tools/#storage-destinations). Retrieve the organization’s external ID with `GET /v1/storage-destinations/aws-external-id` or `get_aws_storage_external_id`; both require `storage_destinations:create_update`.

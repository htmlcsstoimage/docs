...
slug: template-editor/advanced/sizing-positioning
description: >-
  Understand block sizing and positioning in the template editor.
---
<a id="sizing-and-positioning" />
...
| Property | Details |
| --- | --- |
| Align to canvas | Aligns a selected block to the canvas |
| Rotation | Rotates the block |
| Flip | Flips the block horizontally or vertically |
| Lock | Prevents accidental movement or selection changes |
| Layer order | Moves a block forward, backward, to the front, or to the back |
| ID | Sets a block ID for advanced targeting |
| Extra CSS | Opens advanced CSS controls for the selected block |

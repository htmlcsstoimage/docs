...
<Aside title="Default resolution">HTML images default to `device_scale: 2`. URL screenshots default to `1`; set `device_scale: 2` when you need a higher-resolution screenshot.</Aside>
...
<Aside title="Performance tip">Start with `device_scale: 2` for web images, then check sharpness and file size at the intended display size.</Aside>
